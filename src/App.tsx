  ...
  return (
    <div style={{ ...Sp, padding:40 }}>
      <div style={{ maxWidth:880, margin:"0 auto" }}>
        <div style={{ fontSize:13, color:GOLD, letterSpacing:4, marginBottom:8, fontWeight:700, fontFamily:"sans-serif" }}>AI CREATOR PLATFORM</div>
        <h1 style={{ fontFamily:"'Cinzel',Georgia,serif", color:GOLD, letterSpacing:4, textTransform:"uppercase", margin:0, fontSize:30, marginBottom:14 }}>MAKE AWESOME FAMILY MOVIES OR TURN YOUR DREAMS INTO REALITY</h1>
        <p style={{ color:WHITE, fontSize:15, lineHeight:1.9, maxWidth:720, marginBottom:28, fontFamily:"'Rajdhani',Arial,sans-serif" }}>MandaStrong Studio combines the power of 600+ professional AI tools with an intuitive cinematic workspace — so anyone can create stunning short films, family videos, or feature-length productions up to 3 hours long. No film school required.</p>
        <div style={{ display:"grid", gridTemplateColumns:"repeat(4,1fr)", gap:12, marginBottom:28 }}>
          {[["600+","AI Tools"],["8K","Export Quality"],["3 HOURS","Max Duration"],["1TB","Cloud Storage"]].map(([v,l]) => (
            <div key={v} style={{ background:"#000", border:`1px solid ${GOLDDIM}`, borderRadius:0, padding:18, textAlign:"center" }}>
              <div style={{ color:GOLD, fontFamily:"'Cinzel',Georgia,serif", fontSize:22, fontWeight:900 }}>{v}</div>
              <div style={{ color:WHITE, fontFamily:"Arial,sans-serif", fontSize:13, marginTop:4, fontWeight:600, letterSpacing:1 }}>{l}</div>
            </div>
          ))}
        </div>
        <div style={{ display:"grid", gridTemplateColumns:"repeat(3,1fr)", gap:16, marginBottom:28 }}>
          {[
            {ic:"🎬", t:"WRITE YOUR SCRIPT", d:"600+ writing tools. Script to movie, dialogue generator, story outline, beat sheets and more. AI writes it with you."},
            {ic:"🎙", t:"ADD YOUR VOICE", d:"Clone your own voice or choose from 6 professional stock voices. Paste your script and hear it spoken instantly."},
            {ic:"🎞", t:"BUILD YOUR FILM", d:"Upload media, drag clips onto the timeline, mix audio, enhance with AI, render up to 8K and export to YouTube."},
          ].map(f => (
            <div key={f.t} style={{ background:"#000", border:`1px solid ${GOLDDIM}`, borderRadius:0, padding:20 }}>
              <div style={{ fontSize:28, marginBottom:10 }}>{f.ic}</div>
              <div style={{ color:GOLD, fontSize:13, fontWeight:900, letterSpacing:2, marginBottom:8, fontFamily:"Arial,sans-serif" }}>{f.t}</div>
              <div style={{ color:WHITE, fontSize:13, lineHeight:1.7, fontFamily:"Arial,sans-serif" }}>{f.d}</div>
            </div>
          ))}
        </div>
        <div style={{ background:"#000", border:`1px solid ${GOLD}`, borderRadius:0, padding:24, marginBottom:28 }}>
          <div style={{ color:GOLD, fontSize:13, fontWeight:900, letterSpacing:3, marginBottom:14, fontFamily:"Arial,sans-serif" }}>WHO IS THIS FOR?</div>
          <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:10 }}>
            {["Families wanting to preserve memories as real films","First-time filmmakers with a story to tell","Authors turning books into documentary films","Educators creating video content","Veterans sharing their stories","Anyone who has ever said — I should make a film about that"].map(i => (
              <div key={i} style={{ color:WHITE, fontSize:13, fontFamily:"Arial,sans-serif", display:"flex", alignItems:"flex-start", gap:8 }}>
                <span style={{ color:GOLD, flexShrink:0 }}>✦</span>{i}
              </div>
            ))}
          </div>
        </div>
        <button onClick={() => go(4)} style={{ background:`linear-gradient(135deg,${GOLDDIM},${GOLD})`, border:"none", color:"#000", borderRadius:0, fontWeight:900, padding:"14px 38px", fontSize:14, cursor:"pointer", letterSpacing:3, textTransform:"uppercase", fontFamily:"Arial,sans-serif" }}>START CREATING</button>
      </div>
    </div>
  );
}
    ...
    <div style={{...Sp,padding:"26px 40px 80px"}}>
      <div style={{maxWidth:780,margin:"0 auto",textAlign:"center"}}>
        ...
        <div style={{...Card(),textAlign:"left",marginBottom:16}}>
          ...
          <p style={{color:GOLD,fontWeight:900,fontSize:14,letterSpacing:2}}>— AMANDA WOOLLEY</p>
...
function speakText(voiceId, txt, onStart, onEnd) {
  ...
  const doSpeak = () => {
    ...
    if (voiceId === "james" || voiceId === "marcus" || voiceId === "river") {
      picked = allVoices.find(x => /david|james|daniel|oliver|arthur|fred|tom|bruce|lee|ryan|eric/i.test(x.name))
            || allVoices.find(x => x.lang==="en-GB" && x.localService)
            || allVoices.find(x => x.lang.startsWith("en") && x.name.toLowerCase().indexOf("female")===-1 && x.name.toLowerCase().indexOf("zira")===-1 && x.name.toLowerCase().indexOf("samantha")===-1)
