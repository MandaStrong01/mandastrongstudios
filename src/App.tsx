        ...
        <div className="flex flex-col gap-4">
          {plans.map(p => (
            <div key={p.name} style={{ border: p.name === "Pro" ? `2px solid ${GOLD}` : `1px solid ${BORDER}`, background: BG3 }} className="rounded-2xl p-5">
              ...
              <ul className="mb-4" style={{ color: TEXT2 }}>{p.features.map(f => <li key={f} style={{ marginBottom: 4 }}>checkmark {f}</li>)}</ul>
...
const AI_TOOLS_ALL = [
  ...
  { page: 8, name: "Upload and Enhance", desc: "Upload footage - AI enhances quality automatically.", icon: "⬆️" },
  ...
  { page: 9, name: "Translate and Dub", desc: "Translate and dub your film into any language.", icon: "🌍" },
...
function AIToolBoard({ pageNum, go, addAsset }: { pageNum: number; go: (n: number) => void; addAsset: (name: string) => void }) {
  ...
  const labels: Record<number, string> = { 4: "Script and Story", 5: "Design and Characters", 6: "Video and VFX", 7: "Writing and Marketing", 8: "Upload and Enhance", 9: "Edit and Export" };
  return (
    <Page>
      <div className="px-4 py-4 max-w-lg mx-auto">
        <SH>AI Tool Board - {labels[pageNum]}</SH>
        <div className="relative mb-4">
          <span style={{ position: "absolute", left: 12, top: "50%", transform: "translateY(-50%)", color: PUR2 }}>search</span>
          <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search 720+ AI tools..." style={{ background: BG3, border: `1px solid ${PUR2}`, color: TEXT, borderRadius: 16, width: "100%", padding: "10px 12px 10px 36px", outline: "none", fontSize: 13 }} />
        </div>
    ...
    <Page>
      <div className="px-4 py-4 max-w-lg mx-auto">
        <div style={{ textAlign: "center", marginBottom: 16 }}>
          ...
          <div style={{ fontSize: 11, letterSpacing: 2, color: TEXT3, marginTop: 3 }}>24/7 PRODUCTION SUPPORT - ONLINE</div>
        </div>
        <div style={{ background: BG2, border: `1px solid ${BORDER}`, borderRadius: 16, padding: 14, minHeight: 260, maxHeight: 360, overflowY: "auto", display: "flex", flexDirection: "column", gap: 10, marginBottom: 12 }}>
          {msgs.length === 0 && <div style={{ padding: "10px 14px", background: "rgba(26,82,118,.2)", borderLeft: "2px solid #2980b9", color: "#aed6f1", fontSize: 13, lineHeight: 1.7, borderRadius: 8 }}>I am Agent Grok - your 24/7 MandaStrong Studio assistant powered by Claude AI. Ask me anything about the platform, your account, AI tools, editing, or filmmaking!</div>}
          {msgs.map((m, i) => (
...
function P20({ go }: { go: (n: number) => void }) {
  ...
  const addPost = (file: File) => {
    ...
    setPosts(prev => [{ id: Date.now(), user: "You", title, desc: "Just uploaded to MandaStrong Community.", likes: 0, videoFile: file, videoUrl: "", comments: [] }, ...prev]);
  };
  ...
  return (
    <Page>
      <div className="px-5 py-6 max-w-lg mx-auto">
        ...
        <h1 style={{ color: GOLD, fontWeight: 900, fontSize: 34, textAlign: "center", textTransform: "uppercase", letterSpacing: 3, marginBottom: 22 }}>That's All Folks!</h1>
        ...
        <div style={{ background: "linear-gradient(135deg, #120d00, #1e1500)", border: `1.5px solid ${GOLDDIM}`, borderRadius: 18, padding: 20, marginBottom: 16 }}>
          ...
          <div style={{ background: "#0d0900", border: `1px solid ${GOLDDIM}`, borderRadius: 14, padding: 14, marginBottom: 12, textAlign: "center" }}>
            <p style={{ color: TEXT, fontSize: 12, lineHeight: 1.8 }}>Fundraiser: Educational Program on Bullying Prevention and Social Skills - Through this comprehensive program, we provide educational resources and movie-based content to help schools address these critical issues. Our goal is to create safe, supportive environments where every student can thrive.</p>
          </div>
          <div style={{ background: "#0d0900", border: `1px solid ${GOLD}`, borderRadius: 14, padding: 14, textAlign: "center" }}>
            ...
            <p style={{ color: TEXT, fontSize: 12, lineHeight: 1.8 }}>All Etsy Store Proceeds Benefit Veterans Mental Health Services - 100% of all proceeds from our Etsy Store fundraiser are donated directly to Veterans Mental Health Services, supporting those who have sacrificed so much for our freedom.</p>
          </div>
        </div>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <ul style={{ color: TEXT2, fontSize: 11, lineHeight: 2.0 }}>
                  <li>Pages 1-3: Welcome, Story, Login</li>
                  <li>Pages 4-9: AI Tool Board 720+ tools</li>
                  <li>Page 10: Upload your existing movie</li>
                  <li>Page 11: Media Box with all assets</li>
                </ul>
              </div>
              <div>
                <div style={{ color: GOLD2, fontWeight: 700, fontSize: 12, marginBottom: 8 }}>Editing and Export</div>
                <ul style={{ color: TEXT2, fontSize: 11, lineHeight: 2.0 }}>
                  <li>Pages 12-16: Editing tools</li>
                  <li>Page 17: Film preview</li>
                  <li>Page 18: Terms of Service</li>
                  <li>Page 19: Agent Grok Help Desk</li>
                  <li>Page 20: Community Hub</li>
                </ul>
              </div>
            </div>
          </div>
...
function QuickAccess({ go, onClose }: { go: (n: number) => void; onClose: () => void }) {
  const links = [
    ...
    { label: "Script and Story", icon: "📝", page: 4 }, { label: "Design", icon: "🎨", page: 5 }, { label: "Video and VFX", icon: "🎬", page: 6 },
    { label: "Writing", icon: "✍️", page: 7 }, { label: "Upload and Enhance", icon: "⬆️", page: 8 }, { label: "Edit and Export", icon: "✂️", page: 9 },
    ...
    { label: "Tutorials", icon: "🎓", page: 16 }, { label: "Film Preview", icon: "▶️", page: 17 }, { label: "Terms and Service", icon: "📄", page: 18 },
    ...
    <div style={{ position: "fixed", inset: 0, zIndex: 200, background: "rgba(0,0,0,0.9)", overflowY: "auto" }} onClick={onClose}>
      <div style={{ background: BG2, margin: "20px 12px", borderRadius: 22, border: `2px solid ${GOLD}`, padding: 20 }} onClick={e => e.stopPropagation()}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 18 }}>
          <div style={{ color: GOLD, fontWeight: 900, fontSize: 18 }}>Admin Dashboard</div>
          <button onClick={onClose} style={{ background: "none", border: "none", color: TEXT3, fontSize: 22, cursor: "pointer" }}>X</button>
        </div>
    ...
    <>
      <div style={{ background: BG2, borderBottom: `1px solid ${BORDER}`, position: "fixed", top: 0, left: 0, right: 0, zIndex: 100, display: "flex", alignItems: "center", justifyContent: "space-between", padding: "6px 12px", gap: 8 }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
          ...
          {user.isAdmin && <button onClick={() => setShowAdmin(true)} style={{ background: "linear-gradient(135deg,#7f1d1d,#991b1b)", border: "1px solid #ef4444", color: "#ef4444", borderRadius: 10, padding: "5px 10px", fontSize: 11, fontWeight: 700, cursor: "pointer" }}>Admin</button>}
