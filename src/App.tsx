            ...
            <div className="bg-zinc-950 border-2 border-[#7c3aed] rounded-3xl p-8 max-w-2xl w-full fade-up">
              ...
              <div className="space-y-5">
                <div className="bg-black border border-[#7c3aed]/30 rounded-xl p-5">
                  ...
                  <button onClick={() => fileInputRef.current?.click()} className="w-full bg-zinc-900 border-2 border-dashed border-[#7c3aed] p-4 rounded-xl text-white hover:bg-[#7c3aed]/10 transition font-bold">📁 BROWSE FILES</button>
                </div>
                <div className="bg-black border border-[#7c3aed]/30 rounded-xl p-5">
            ...
            <div className="text-center max-w-5xl w-full">
              ...
              <div className="grid grid-cols-3 gap-4">
                {[{icon:Database,label:'Cloud Storage',info:'1TB Included',sub:'Auto-backup enabled'},
                  ...
                  {icon:Shield,label:'Secure',info:'256-bit Encryption',sub:'Your files are private'}].map(({icon:Icon,label,info,sub}) => (
          ...
          <div className="min-h-screen p-8 pt-20 pb-40 fade-up">
            ...
            <div className="grid md:grid-cols-3 gap-6 max-w-6xl mx-auto mb-8">
              {[{icon:Database,label:'Media Library',sub:`${mediaLibrary.length} assets ready`,p:12,info:'Drag to timeline'},
                ...
                {icon:Sliders,label:'Audio Mixer',sub:'4 channels',p:14,info:'Music • Voice • SFX • Master'}].map(({icon:Icon,label,sub,p,info}) => (
            ...
            <div className="grid md:grid-cols-3 gap-6 max-w-6xl mx-auto">
              {[{icon:FileVideo,label:'Render Film',sub:'Start final render',p:15,info:'Up to 8K quality'},
                ...
                {icon:Download,label:'Export & Share',sub:'Download or share',p:17,info:'MP4 • MOV • AVI'}].map(({icon:Icon,label,sub,p,info}) => (
                <button key={label} onClick={() => goTo(p)} className="bg-zinc-950 border-2 border-[#7c3aed]/40 p-8 rounded-3xl hover:bg-[#7c3aed]/10 hover:border-[#7c3aed] transition text-left group">
                  <Icon size={48} className="text-[#7c3aed] mb-4 group-hover:scale-110 transition" />
                  <h3 className="text-xl font-black uppercase mb-1 text-white">{label}</h3>
                  <p className="text-sm text-zinc-400">{sub}</p>
