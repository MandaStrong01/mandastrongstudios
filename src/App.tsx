...
const Spage = { minHeight:"100vh", background:BG, color:WHITE, fontFamily:"'Rajdhani',sans-serif", paddingBottom:70 };
const Sh1 = { fontFamily:"'Cinzel',serif", color:GOLD, letterSpacing:4, textTransform:"uppercase", margin:0 };
const Scard = (extra) => ({ background:BG3, border:`1px solid ${GOLDDIM}44`, borderRadius:14, padding:20, ...(extra||{}) });
  ...
  return (
    <div style={{ position:"fixed", inset:0, zIndex:1000, display:"flex" }}>
      <div style={{ width:260, background:BG2, borderRight:`2px solid ${GOLD}`, height:"100vh", overflowY:"auto", padding:20, boxShadow:`4px 0 40px ${GOLD}44` }}>
        ...
        <div style={{ background:`linear-gradient(135deg,${GOLDDIM},${GOLD})`, borderRadius:10, padding:"10px 14px", marginBottom:8, textAlign:"center" }}>
        ...
        {user && user.plan && (
          <div style={{ background:BG3, border:`1px solid ${GOLDDIM}`, borderRadius:8, padding:"8px 12px", marginBottom:12, textAlign:"center" }}>
            <div style={{ color:TEXT3, fontSize:9, letterSpacing:1 }}>CURRENT PLAN</div>
            <div style={{ color:GOLD, fontWeight:800, fontSize:15 }}>{user.plan}</div>
          </div>
        )}
        {NAV_PAGES.map(i => (
          <button key={i.p} onClick={() => { go(i.p); onClose(); }}
            style={{ width:"100%", textAlign:"left", background:"none", border:"none", color:WHITE, padding:"9px 10px", borderRadius:8, cursor:"pointer", fontSize:13, fontWeight:700, display:"block", marginBottom:1 }}
...
function Header({ go, setMenuOpen }) {
  return (
    <header style={{ position:"sticky", top:0, zIndex:500, background:BG2, borderBottom:`1px solid ${GOLDDIM}44`, padding:"0 14px", height:56, display:"flex", alignItems:"center", gap:10, boxShadow:`0 2px 24px ${GOLDDIM}33` }}>
      <button onClick={() => setMenuOpen(true)} style={{ background:"none", border:`1px solid ${GOLDDIM}`, color:GOLD, borderRadius:8, width:36, height:36, cursor:"pointer", fontSize:18, flexShrink:0 }}>☰</button>
      ...
      <div style={{ flex:1, overflow:"hidden", margin:"0 8px" }}>
        <div style={{ color:TEXT3, fontSize:10, letterSpacing:1, whiteSpace:"nowrap", animation:"ticker 22s linear infinite" }}>
          ✦ Special Offer: New Studio Plan Subscribers Receive 7-Day Free Trial &nbsp;·&nbsp; 600+ AI TOOLS &nbsp;·&nbsp; 8K EXPORT &nbsp;·&nbsp; UP TO 3-HOUR FILMS &nbsp;·&nbsp; PROFESSIONAL CINEMA SYNTHESIS &nbsp;·&nbsp; ✦ Special Offer: New Studio Plan Subscribers Receive 7-Day Free Trial &nbsp;·&nbsp; 600+ AI TOOLS &nbsp;·&nbsp;
        </div>
      </div>
      <div style={{ display:"flex", alignItems:"center", gap:8, flexShrink:0 }}>
        <div style={{ color:"#22c55e", fontSize:12, letterSpacing:2, fontWeight:800 }}>● SYSTEM ONLINE</div>
        <div onClick={() => go(21)} style={{ width:40, height:40, background:`linear-gradient(135deg,${GOLDDIM},${GOLD})`, borderRadius:10, display:"flex", alignItems:"center", justifyContent:"center", cursor:"pointer", fontFamily:"'Cinzel',serif", fontSize:22, fontWeight:900, color:"#000", boxShadow:`0 0 12px ${GOLD}66` }}>G</div>
      </div>
...
function ToolCard({ name }) {
  ...
  return (
    <div style={{ background:BG2, border:`1px solid ${GOLDDIM}22`, borderRadius:10, padding:"11px 10px", display:"flex", flexDirection:"column", gap:7, transition:"border-color .2s" }}
      ...
      <div style={{ display:"flex", gap:4 }}>
        <button onClick={() => fileRef.current && fileRef.current.click()} style={{ fontSize:9, padding:"3px 7px", background:BG3, border:`1px solid ${GOLDDIM}44`, color:WHITE, borderRadius:5, cursor:"pointer", fontWeight:700, letterSpacing:1 }}>UPLOAD</button>
        <button style={{ fontSize:9, padding:"3px 7px", background:BG3, border:`1px solid ${GOLDDIM}44`, color:WHITE, borderRadius:5, cursor:"pointer", fontWeight:700, letterSpacing:1 }}>PASTE</button>
        <button onClick={doAI} style={{ fontSize:9, padding:"3px 8px", background:GOLDDIM, border:`1px solid ${GOLD}`, color:"#000", borderRadius:5, cursor:"pointer", fontWeight:900 }}>{loading ? "..." : "AI ✦"}</button>
      </div>
        ...
        <div style={{ display:"flex", alignItems:"center", gap:8 }}>
          <div style={{ position:"relative" }}>
            <input value={search} onChange={e => setSearch(e.target.value)} placeholder={`Search ${tools.length} tools...`}
              style={{ background:BG3, border:`1px solid ${GOLDDIM}`, borderRadius:8, padding:"7px 12px 7px 30px", color:WHITE, fontSize:12, outline:"none", width:200 }} />
...
function P1({ go }) {
  return (
    <div style={{ ...Spage }}>
      <div style={{ background:"linear-gradient(180deg,#0d0d0d,#111)", padding:"56px 40px 36px", textAlign:"center", borderBottom:`1px solid ${GOLDDIM}33` }}>
        <div style={{ fontSize:9, color:TEXT3, letterSpacing:5, marginBottom:10 }}>CINEMA INTELLIGENCE PLATFORM — EST. 2026</div>
        <div style={{ fontFamily:"'Cinzel',serif", fontSize:"clamp(32px,6vw,52px)", fontWeight:900, color:GOLD, letterSpacing:6, lineHeight:1.1, textShadow:`0 0 40px ${GOLD}99, 0 0 80px ${GOLD}44` }}>MANDA STRONG</div>
        <div style={{ fontFamily:"'Cinzel',serif", fontSize:"clamp(32px,6vw,52px)", fontWeight:900, color:GOLD, letterSpacing:6, lineHeight:1.1, textShadow:`0 0 40px ${GOLD}99, 0 0 80px ${GOLD}44`, marginBottom:10 }}>STUDIO</div>
        <div style={{ color:WHITE, fontSize:12, letterSpacing:2, marginBottom:20, fontWeight:600 }}>600+ AI TOOLS · 8K EXPORT · UP TO 3-HOUR FILMS</div>
        <div style={{ display:"flex", gap:12, justifyContent:"center", flexWrap:"wrap" }}>
          <button onClick={() => go(4)} style={{ ...Sbtn("gold",false), fontSize:15, padding:"14px 32px" }}>START CREATING</button>
          <button onClick={() => go(4)} style={{ ...Sbtn("out",false), fontSize:15, padding:"14px 32px" }}>LOGIN / REGISTER</button>
        </div>
      </div>
      ...
      <div style={{ textAlign:"center", padding:"0 32px 20px" }}>
        <button onClick={() => alert("To install MandaStrong Studio:\n\nMobile: Tap Share then Add to Home Screen\nDesktop: Click the install icon in your browser address bar")} style={{ ...Sbtn("out",true), fontSize:11 }}>⬇ Download As App</button>
      </div>
      <div style={{ background:BG3, borderTop:`1px solid ${GOLDDIM}22`, borderBottom:`1px solid ${GOLDDIM}22`, padding:"7px 0", overflow:"hidden" }}>
        <div style={{ whiteSpace:"nowrap", animation:"ticker 28s linear infinite", color:TEXT3, fontSize:10, letterSpacing:1 }}>
  ...
  return (
    <div style={{ ...Spage, padding:40 }}>
      <div style={{ maxWidth:900, margin:"0 auto" }}>
        ...
        <h1 style={{ ...Sh1, fontSize:32, marginBottom:12 }}>MAKE AWESOME FAMILY MOVIES OR TURN YOUR DREAMS INTO REALITY</h1>
  ...
  return (
    <div style={{ ...Spage, padding:40 }}>
      <div style={{ maxWidth:900, margin:"0 auto" }}>
        ...
        <h1 style={{ ...Sh1, fontSize:30, marginBottom:24 }}>EXAMPLES MADE BY MANDASTRONG STUDIO</h1>
        <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:20 }}>
          {[{s:1,d:"A documentary film — MandaStrong Studio x Doxy"},{s:2,d:"A plain-English guide to artificial intelligence"}].map(f => (
            <div key={f.s} style={{ ...Scard() }}>
              <div style={{ background:"#000", borderRadius:10, height:160, display:"flex", alignItems:"center", justifyContent:"center", marginBottom:12, fontSize:40, color:TEXT3 }}>🎬</div>
              <div style={{ fontSize:9, color:GOLD, letterSpacing:2, fontWeight:700, marginBottom:6 }}>VIEWER 0{f.s}</div>
...
function P4({ go, setUser }) {
  ...
  const inp = { width:"100%", background:BG, border:`1px solid ${GOLDDIM}`, borderRadius:8, padding:"10px 12px", color:WHITE, fontSize:14, marginBottom:10, outline:"none", boxSizing:"border-box" };
  const login = () => {
      ...
      <div style={{ maxWidth:1000, margin:"0 auto" }}>
        <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr 1fr", gap:20, marginBottom:40 }}>
          <div style={{ ...Scard() }}>
            ...
            <div style={{ textAlign:"center", marginTop:8, color:TEXT3, fontSize:10 }}>Secured with 256-bit encryption</div>
          </div>
          <div style={{ ...Scard(), border:"2px solid #22c55e", position:"relative" }}>
            <div style={{ position:"absolute", top:-12, left:"50%", transform:"translateX(-50%)", background:"#22c55e", color:"#000", borderRadius:20, padding:"3px 14px", fontSize:10, fontWeight:900, whiteSpace:"nowrap" }}>🎉 7-DAY FREE TRIAL</div>
            ...
            <button onClick={() => { setUser({ name:name||"Creator", plan:"Studio Trial", isAdmin:false }); window.open(STRIPE.studio,"_blank"); go(5); }}
              style={{ width:"100%", padding:"13px", background:"#22c55e", border:"none", color:"#000", borderRadius:10, fontWeight:900, fontSize:14, cursor:"pointer", letterSpacing:1 }}>START FREE TRIAL — $0</button>
            <div style={{ textAlign:"center", marginTop:8, color:TEXT3, fontSize:10 }}>Studio Plan Free for 7 Days · No Credit Card</div>
          </div>
        ...
        <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr 1fr", gap:18 }}>
          {[
            ...
            <div key={plan.t} style={{ ...Scard(), border:plan.pop?`2px solid ${GOLD}`:`1px solid ${GOLDDIM}33`, position:"relative" }}>
              {plan.pop && <div style={{ position:"absolute", top:-11, left:"50%", transform:"translateX(-50%)", background:GOLD, color:"#000", borderRadius:20, padding:"2px 12px", fontSize:10, fontWeight:900, whiteSpace:"nowrap" }}>MOST POPULAR</div>}
              {plan.trial && <div style={{ position:"absolute", top:-11, right:12, background:"#22c55e", color:"#000", borderRadius:20, padding:"2px 10px", fontSize:10, fontWeight:900, whiteSpace:"nowrap" }}>🎉 FREE TRIAL</div>}
    ...
    <div style={{ ...Spage, padding:40 }}>
      <div style={{ maxWidth:800, margin:"0 auto" }}>
        ...
        <div onDragOver={e => { e.preventDefault(); e.currentTarget.style.borderColor=GOLD; }}
          ...
          style={{ border:`2px dashed ${GOLDDIM}`, borderRadius:14, padding:"56px 40px", textAlign:"center", cursor:"pointer", marginBottom:18, transition:"border-color .2s" }}>
          <div style={{ fontSize:40, marginBottom:10 }}>🎬</div>
        ...
        <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr 1fr", gap:12, marginBottom:22 }}>
          {[["📁","BROWSE FILES"],["🖥","RECORD SCREEN"],["🔗","IMPORT FROM URL"]].map(([ic,lb]) => (
            <button key={lb} onClick={() => fileRef.current && fileRef.current.click()}
              style={{ ...Scard(), textAlign:"center", padding:18, cursor:"pointer", display:"block", border:`1px solid ${GOLDDIM}44` }}>
            ...
            <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fill,minmax(150px,1fr))", gap:10 }}>
              {mediaLib.map(a => (
                <div key={a.id} style={{ ...Scard(), padding:10, position:"relative" }}>
                  {a.type.startsWith("video") ? <video src={a.url} style={{ width:"100%", borderRadius:6, marginBottom:6 }} /> :
                   a.type.startsWith("image") ? <img src={a.url} style={{ width:"100%", borderRadius:6, marginBottom:6 }} alt={a.name} /> :
                   <div style={{ height:70, background:BG, borderRadius:6, marginBottom:6, display:"flex", alignItems:"center", justifyContent:"center", fontSize:28 }}>🎵</div>}
...
function P13({ go, mediaLib, timeline, setTimeline }) {
  ...
  return (
    <div style={{ ...Spage, padding:20 }}>
      ...
      <div style={{ background:"#000", borderRadius:10, height:110, display:"flex", alignItems:"center", justifyContent:"center", marginBottom:14, border:`1px solid ${GOLDDIM}22` }}>
        {mediaLib[0] && mediaLib[0].type.startsWith("video") ?
          ...
          <div style={{ textAlign:"center", color:TEXT3 }}>
            <div style={{ fontSize:12, letterSpacing:2, marginBottom:8, color:WHITE }}>ADD MEDIA TO SEE PREVIEW</div>
      ...
      {tracks.map((tr, idx) => (
        <div key={idx} style={{ marginBottom:10 }}>
          ...
          <div onDragOver={e => e.preventDefault()}
            ...
            style={{ background:BG3, border:`1px dashed ${GOLDDIM}44`, borderRadius:8, minHeight:46, padding:8, display:"flex", gap:8, alignItems:"center", flexWrap:"wrap" }}>
            {(timeline[idx]||[]).map((a,i) => (
              <div key={i} style={{ background:GOLDDIM, borderRadius:6, padding:"4px 10px", fontSize:11, color:"#000", fontWeight:700, display:"flex", alignItems:"center", gap:6 }}>
                {a.name.slice(0,12)}
          ...
          <div style={{ display:"flex", gap:8, flexWrap:"wrap" }}>
            {mediaLib.map(a => (
              <div key={a.id} draggable onDragStart={e => e.dataTransfer.setData("assetId",String(a.id))}
                style={{ background:BG3, border:`1px solid ${GOLDDIM}`, borderRadius:6, padding:"5px 12px", cursor:"grab", color:GOLD, fontSize:11, fontWeight:700 }}>
                📎 {a.name.slice(0,14)}
              </div>
            ))}
          </div>
      ...
      <div style={{ width:180, background:BG2, borderRight:`1px solid ${GOLDDIM}22`, overflowY:"auto", padding:10 }}>
        {tools14.map(t => (
          <button key={t} onClick={() => setActive(t)}
            style={{ width:"100%", textAlign:"left", background:t===active?BG3:"none", border:"none", color:t===active?GOLD:WHITE, padding:"8px 10px", borderRadius:6, cursor:"pointer", fontSize:12, fontWeight:t===active?900:600, marginBottom:2 }}>
            {t}
          </button>
        ))}
      </div>
  ...
  return (
    <div style={{ ...Spage, padding:40 }}>
      <div style={{ maxWidth:800, margin:"0 auto" }}>
        ...
        {done && (
          <div style={{ background:"#14532d", border:"1px solid #22c55e", borderRadius:12, padding:16, marginBottom:14, textAlign:"center" }}>
  ...
  return (
    <div style={{ ...Spage, padding:40 }}>
      <div style={{ maxWidth:900, margin:"0 auto" }}>
        ...
        <div style={{ background:"#000", borderRadius:12, overflow:"hidden", marginBottom:14, aspectRatio:"16/9", display:"flex", alignItems:"center", justifyContent:"center" }}>
          {videoSrc ?
            <video src={videoSrc} controls style={{ width:"100%", height:"100%" }} /> :
            <div style={{ textAlign:"center", color:TEXT3 }}>
              <div style={{ fontSize:40, marginBottom:10 }}>🎬</div>
              <div style={{ fontSize:12, letterSpacing:2, color:WHITE }}>NO RENDER AVAILABLE</div>
      ...
      <div style={{ maxWidth:800, margin:"0 auto" }}>
        ...
        <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr 1fr", gap:12, marginBottom:22 }}>
          {[["💾","DOWNLOAD TO DEVICE",download],["💿","SAVE PROJECT FILE",()=>{}],["🌐","SHARE TO COMMUNITY HUB",()=>{}]].map(([ic,lb,fn]) => (
            <button key={lb} onClick={fn} style={{ ...Scard(), cursor:"pointer", textAlign:"center", padding:18, display:"block", border:`1px solid ${GOLDDIM}44` }}>
              <div style={{ fontSize:26, marginBottom:6 }}>{ic}</div>
              <div style={{ color:WHITE, fontSize:11, fontWeight:800, letterSpacing:1 }}>{lb}</div>
            </button>
          ))}
      ...
      <div style={{ maxWidth:800, margin:"0 auto" }}>
        ...
        {tuts.map(t => (
          <div key={t.n} onClick={() => window.open("https://youtube.com","_blank")}
            ...
            <span style={{ background:lc[t.l]+"22", border:`1px solid ${lc[t.l]}`, color:lc[t.l], borderRadius:6, padding:"3px 10px", fontSize:10, fontWeight:700, flexShrink:0 }}>{t.l.toUpperCase()}</span>
          </div>
        ))}
      </div>
    </div>
    ...
    <div style={{ ...Spage, padding:40 }}>
      <div style={{ maxWidth:700, margin:"0 auto" }}>
        <div style={{ textAlign:"center", marginBottom:22 }}>
          <div style={{ width:56, height:56, background:`linear-gradient(135deg,${GOLDDIM},${GOLD})`, borderRadius:12, display:"flex", alignItems:"center", justifyContent:"center", margin:"0 auto 12px", fontFamily:"'Cinzel',serif", fontSize:28, fontWeight:900, color:"#000" }}>G</div>
        ...
        <div style={{ ...Scard(), height:300, overflowY:"auto", marginBottom:12, display:"flex", flexDirection:"column", gap:10, padding:14 }}>
          {msgs.map((m,i) => (
            <div key={i} style={{ padding:"11px 14px", borderRadius:10, background:m.role==="user"?"rgba(212,168,71,0.1)":"rgba(26,82,118,0.2)", borderLeft:`3px solid ${m.role==="user"?GOLDDIM:"#2980b9"}` }}>
          ...
          {loading && <div style={{ padding:"11px 14px", background:"rgba(26,82,118,0.2)", borderLeft:"3px solid #2980b9", borderRadius:10, color:WHITE, fontSize:13 }}>Agent Grok is thinking...</div>}
          <div ref={bottomRef} />
        </div>
        ...
        <div style={{ display:"flex", gap:10 }}>
          <textarea value={inp} onChange={e => setInp(e.target.value)}
            ...
            style={{ flex:1, height:52, resize:"none", padding:"12px 14px", fontSize:14, background:BG3, border:`1px solid ${GOLDDIM}`, color:WHITE, borderRadius:10, outline:"none", lineHeight:1.5 }} />
    ...
    <div style={{ ...Spage, padding:"30px 40px 80px" }}>
      <div style={{ maxWidth:800, margin:"0 auto", textAlign:"center" }}>
        ...
        <div style={{ ...Scard(), textAlign:"left", marginBottom:20 }}>
          ...
          <p style={{ color:WHITE, fontSize:14, lineHeight:1.9 }}>Together, we are building a community of creators who use their talents to spread kindness, understanding, and hope. Your creativity and passion inspire positive change in the world.</p>
          <p style={{ color:WHITE, fontSize:14, lineHeight:1.9 }}>With gratitude and cinematic love,</p>
          <p style={{ color:GOLD, fontWeight:900, fontSize:15 }}>— AMANDA STRONG</p>
          <p style={{ color:WHITE, fontSize:12 }}>Founder, MandaStrong Studio · Author of Doxy the School Bully<br/>MandaStrong1.Etsy.com</p>
        </div>
        <div style={{ ...Scard(), textAlign:"left", marginBottom:20 }}>
          ...
          <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:12 }}>
            {[
              ...
              <div key={m.t} style={{ background:BG2, border:`1px solid ${GOLDDIM}44`, borderRadius:10, padding:14 }}>
...
export default function App() {
  ...
  const pages = {
    1:<P1 go={go} />, 2:<P2 go={go} />, 3:<P3 go={go} />, 4:<P4 go={go} setUser={setUser} />,
    ...
    19:<P19 />, 20:<P20 />, 21:<P21 />, 22:<P22 />,
    23:<P23 go={go} />,
  };
  return (
    <div style={{ background:BG, minHeight:"100vh", fontFamily:"'Rajdhani',sans-serif" }}>
      ...
      <Header go={go} setMenuOpen={setMenuOpen} />
