...
function P1({ go }: { go: (n: number) => void }) {
  return (
    <div style={{ background:"radial-gradient(ellipse at 50% 30%, #1a1200 0%, #0d0900 50%, #000 100%)", minHeight:"100vh", paddingTop:56, position:"relative", overflow:"hidden" }}>
      <Sparkles />
      <div style={{ position:"absolute", top:"15%", left:"50%", transform:"translateX(-50%)", width:500, height:500, borderRadius:"50%", background:"radial-gradient(circle, #d4a84715 0%, transparent 70%)", pointerEvents:"none" }} />
      <div style={{ position:"relative", zIndex:1, display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", minHeight:"calc(100vh - 56px)", padding:"0 24px", textAlign:"center" }}>
        <div style={{ fontSize:56, marginBottom:10, filter:"drop-shadow(0 0 20px #d4a847)", animation:"float 4s ease-in-out infinite" }}>✦</div>
        <div style={{ color:TEXT3, fontSize:12, letterSpacing:5, textTransform:"uppercase", marginBottom:16 }}>Cinema Intelligence Platform</div>
        <h1 style={{ color:GOLD, fontSize:50, fontWeight:900, letterSpacing:5, textTransform:"uppercase", marginBottom:12, lineHeight:1.0, animation:"glowPulse 3s ease-in-out infinite" }}>
          MandaStrong<br />Studio
        </h1>
        <p style={{ color:TEXT2, fontSize:16, marginBottom:10, letterSpacing:1, maxWidth:340 }}>Welcome To The All In One Make Your Own Longer Movies App!</p>
        <div style={{ color:GOLDDIM, fontSize:13, marginBottom:28, letterSpacing:2 }}>600+ AI TOOLS &nbsp;|&nbsp; 8K EXPORT &nbsp;|&nbsp; UP TO 3-HOUR FILMS</div>
        <div style={{ display:"flex", gap:16, marginBottom:36 }}>
          {[["600+","AI Tools"],["8K","Cinema Export"],["3HRS","Max Duration"]].map(([v,l]) => (
            <div key={l} style={{ background:"rgba(212,168,71,0.08)", border:"1px solid "+GOLDDIM, borderRadius:14, padding:"14px 20px", textAlign:"center", minWidth:80 }}>
              <div style={{ color:GOLD, fontWeight:900, fontSize:26 }}>{v}</div>
        ...
        <div style={{ display:"flex", gap:20, marginTop:28 }}>
          <button onClick={() => go(19)} style={{ background:"transparent", border:"1px solid #333", borderRadius:20, padding:"7px 18px", color:TEXT3, fontSize:12, cursor:"pointer", letterSpacing:1 }}>Agent Grok &nbsp;<span style={{ color:"#2ecc71" }}>● ONLINE</span></button>
          <button onClick={() => go(3)} style={{ background:"transparent", border:"1px solid #333", borderRadius:20, padding:"7px 18px", color:TEXT3, fontSize:12, cursor:"pointer", letterSpacing:1 }}>Login / Register</button>
        </div>
      </div>
    </div>
  );
  ...
  return (
    <div style={{ background:"radial-gradient(ellipse at center,#1a1200 0%,#0a0800 40%,#000 100%)", minHeight:"100vh", display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", padding:24, position:"relative" }}>
      ...
      <div style={{ position:"relative", zIndex:1, width:"100%", maxWidth:400, display:"flex", flexDirection:"column", alignItems:"center" }}>
        ...
        <h1 style={{ color:GOLD, fontSize:34, fontWeight:900, letterSpacing:4, textTransform:"uppercase", textAlign:"center", marginBottom:6, textShadow:"0 0 24px #d4a847", lineHeight:1.1 }}>MandaStrong<br />Studio</h1>
        ...
        <div style={{ background:BG3, border:"1.5px solid "+GOLDDIM, borderRadius:24, padding:28, width:"100%", boxShadow:"0 0 40px "+GOLDDIM+"44" }}>
          <div style={{ display:"flex", marginBottom:24, background:BG2, borderRadius:14, padding:4 }}>
            {(["login","register"] as const).map(m => (
              <button key={m} onClick={()=>{setMode(m);setErr("");}} style={{ flex:1, background:mode===m?GOLD:"transparent", color:mode===m?"#000":TEXT2, border:"none", borderRadius:10, padding:"10px 0", fontWeight:800, fontSize:15, cursor:"pointer", transition:"all 0.2s" }}>
          ...
          {mode==="register" && (
            <div style={{ marginBottom:22 }}>
              ...
              <div style={{ display:"flex", gap:8 }}>
                {plans.map(p=>(<button key={p} onClick={()=>setPlan(p)} style={{ flex:1, background:plan===p?GOLD:BG2, color:plan===p?"#000":TEXT2, border:"1px solid "+(plan===p?GOLD:BORDER), borderRadius:10, padding:"9px 0", fontSize:13, fontWeight:700, cursor:"pointer" }}>{p}</button>))}
              </div>
              <div style={{ display:"flex", justifyContent:"space-between", marginTop:6 }}>
                <span style={{ color:TEXT3, fontSize:12 }}>Basic $10/mo</span><span style={{ color:TEXT3, fontSize:12 }}>Pro $20/mo</span><span style={{ color:TEXT3, fontSize:12 }}>Studio $30/mo</span>
              </div>
  ...
  if (mode==="login"||mode==="register") return (
    <div style={{ background:"radial-gradient(ellipse at center,#1a1200 0%,#0a0800 40%,#000 100%)", minHeight:"100vh", paddingTop:56, position:"relative" }}>
      ...
      <div style={{ position:"relative", zIndex:1, display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", minHeight:"calc(100vh - 56px)", padding:"0 20px" }}>
        ...
        <h2 style={{ color:GOLD, fontSize:28, fontWeight:900, letterSpacing:4, textTransform:"uppercase", textAlign:"center", marginBottom:4, textShadow:"0 0 20px #d4a847" }}>MandaStrong Studio</h2>
    ...
    <Page sparkle>
      <div style={{ padding:"24px 20px", maxWidth:560, margin:"0 auto" }}>
        ...
        <div onDragOver={e=>{e.preventDefault();setDragging(true);}} onDragLeave={()=>setDragging(false)} onDrop={e=>{e.preventDefault();setDragging(false);const f=e.dataTransfer.files[0];if(f)handleFile(f);}} onClick={()=>ref.current?.click()} style={{ border:"2px dashed "+(dragging?GOLD:PUR2), background:dragging?"#1a0d40":BG3, borderRadius:22, padding:48, textAlign:"center", cursor:"pointer", transition:"all 0.3s", boxShadow:dragging?"0 0 30px "+PUR2+"44":"none", marginBottom:20 }}>
          ...
          {file ? (<div><p style={{ color:GOLD, fontWeight:700, fontSize:18, marginBottom:6 }}>{file.name}</p><p style={{ color:TEXT2, fontSize:14 }}>{(file.size/1024/1024).toFixed(1)} MB</p></div>) : (<><p style={{ color:TEXT, fontWeight:700, marginBottom:8, fontSize:18 }}>Drag & Drop Your Film Here</p><p style={{ color:TEXT3, fontSize:15 }}>or tap to browse your device</p><p style={{ color:TEXT3, fontSize:13, marginTop:10 }}>Supports: MP4, MOV, AVI, MKV, ProRes, DNxHD</p><p style={{ color:GOLDDIM, fontSize:13, marginTop:6 }}>Max size: 50GB &bull; Up to 3-hour films</p></>)}
        ...
        <div style={{ display:"flex", flexDirection:"column", gap:12 }}>
          ...
          <div style={{ background:BG3, border:"1px solid "+BORDER, borderRadius:14, padding:16, textAlign:"center" }}>
            ...
            <div style={{ display:"flex", gap:10, justifyContent:"center", flexWrap:"wrap" }}>
              {["Record Screen","Record Camera","Import from Cloud","Import from URL"].map(opt=>(<button key={opt} onClick={()=>alert("Coming soon: "+opt)} style={{ background:BG4, border:"1px solid "+BORDER, color:TEXT2, borderRadius:10, padding:"8px 12px", fontSize:12, cursor:"pointer" }}>{opt}</button>))}
            </div>
          </div>
        </div>
      </div>
    ...
    <Page sparkle>
      <div style={{ padding:"16px", maxWidth:600, margin:"0 auto" }}>
        ...
        <div style={{ display:"flex", gap:8, flexWrap:"wrap", marginBottom:16 }}>
          {types.map(t=>(<button key={t} onClick={()=>setFilter(t)} style={{ background:filter===t?GOLD:BG3, color:filter===t?"#000":TEXT2, border:"1px solid "+(filter===t?GOLD:BORDER), borderRadius:10, padding:"6px 14px", fontSize:14, cursor:"pointer", fontWeight:700 }}>{t}</button>))}
        </div>
    ...
    <Page sparkle>
      <div style={{ padding:"16px", maxWidth:600, margin:"0 auto" }}>
        ...
        <div style={{ display:"flex", gap:8, overflowX:"auto", paddingBottom:10, marginBottom:18 }}>
          {tabs.map(t=>(<button key={t} onClick={()=>setTab(t)} style={{ background:tab===t?GOLD:BG3, color:tab===t?"#000":TEXT2, border:"1px solid "+(tab===t?GOLD:BORDER), borderRadius:12, padding:"8px 18px", fontSize:15, cursor:"pointer", fontWeight:700, whiteSpace:"nowrap" }}>{t}</button>))}
        </div>
  ...
  return (
    <div style={{ background:"radial-gradient(ellipse at 50% 20%, #1a1200 0%, #0d0900 50%, #000 100%)", minHeight:"100vh", paddingTop:56, position:"relative", overflow:"hidden" }}>
      ...
      <div style={{ position:"relative", zIndex:1, padding:"24px 20px", maxWidth:560, margin:"0 auto" }}>
        ...
        <h1 style={{ color:GOLD, fontWeight:900, fontSize:38, textAlign:"center", textTransform:"uppercase", letterSpacing:4, marginBottom:6, animation:"glowPulse 3s ease-in-out infinite" }}>That's All Folks!</h1>
...
function AdminDashboard({ onClose }: { onClose:()=>void }) {
  const stats = [{ label:"Total Users", value:"1,284", icon:"👥" },{ label:"Active Subs", value:"847", icon:"💳" },{ label:"Films Created", value:"3,921", icon:"🎬" },{ label:"Revenue MRR", value:"$16,940", icon:"💰" },{ label:"Basic Plans", value:"312", icon:"📗" },{ label:"Pro Plans", value:"428", icon:"📘" },{ label:"Studio Plans", value:"107", icon:"📕" },{ label:"Community Films", value:"659", icon:"📣" }];
  return (
    <div style={{ position:"fixed", inset:0, zIndex:200, background:"rgba(0,0,0,0.95)", overflowY:"auto" }} onClick={onClose}>
      <div style={{ background:BG2, margin:"18px 12px", borderRadius:24, border:"2px solid "+GOLD, padding:22, boxShadow:"0 0 50px "+GOLDDIM+"66" }} onClick={e=>e.stopPropagation()}>
        ...
        <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:12, marginBottom:22 }}>{stats.map(s=>(<div key={s.label} style={{ background:BG3, border:"1px solid "+GOLDDIM, borderRadius:16, padding:16 }}><div style={{ fontSize:24, marginBottom:6 }}>{s.icon}</div><div style={{ color:GOLD2, fontWeight:900, fontSize:22 }}>{s.value}</div><div style={{ color:TEXT3, fontSize:13 }}>{s.label}</div></div>))}</div>
        <div style={{ background:BG3, border:"1px solid "+GOLDDIM, borderRadius:16, padding:18, marginBottom:20 }}>
          <div style={{ color:GOLD, fontWeight:700, marginBottom:12, fontSize:16 }}>Stripe Revenue - This Month</div>
          <div style={{ display:"flex", gap:6, alignItems:"flex-end", height:70 }}>{[40,55,45,70,60,80,75,90,85,95,88,100].map((h,i)=>(<div key={i} style={{ flex:1, background:"linear-gradient(to top,"+PUR2+","+GOLD+")", borderRadius:4, height:h+"%", opacity:0.8 }} />))}</div>
          <div style={{ display:"flex", justifyContent:"space-between", marginTop:8, color:TEXT3, fontSize:12 }}><span>Jan</span><span>Dec</span></div>
        </div>
        <div style={{ color:GOLD, fontWeight:700, marginBottom:12, fontSize:16 }}>Recent Sign-ups</div>
        <div style={{ display:"flex", flexDirection:"column", gap:8, marginBottom:20 }}>
          {[{ name:"Jordan M.", plan:"Pro", joined:"Today", country:"🇦🇺" },{ name:"Sam K.", plan:"Studio", joined:"Yesterday", country:"🇺🇸" },{ name:"Taylor R.", plan:"Basic", joined:"2 days ago", country:"🇬🇧" },{ name:"Alex P.", plan:"Pro", joined:"3 days ago", country:"🇨🇦" },{ name:"Morgan L.", plan:"Studio", joined:"4 days ago", country:"🇳🇿" }].map(u=>(<div key={u.name} style={{ background:BG3, border:"1px solid "+BORDER, borderRadius:12, padding:"10px 14px", display:"flex", justifyContent:"space-between", alignItems:"center" }}><div style={{ display:"flex", alignItems:"center", gap:10 }}><span style={{ fontSize:18 }}>{u.country}</span><div><div style={{ color:TEXT, fontWeight:600, fontSize:14 }}>{u.name}</div><div style={{ color:TEXT3, fontSize:12 }}>{u.joined}</div></div></div><div style={{ background:u.plan==="Studio"?GOLDDIM:u.plan==="Pro"?PUR2:BG4, color:u.plan==="Studio"?GOLD2:"#fff", borderRadius:8, padding:"3px 10px", fontSize:12, fontWeight:700 }}>{u.plan}</div></div>))}
        </div>
    ...
    <>
      <div style={{ background:BG2, borderBottom:"1px solid "+GOLDDIM, position:"fixed", top:0, left:0, right:0, zIndex:100, display:"flex", alignItems:"center", justifyContent:"space-between", padding:"8px 14px", gap:8, boxShadow:"0 2px 24px "+GOLDDIM+"44" }}>
        ...
        <div style={{ display:"flex", flexDirection:"column", alignItems:"center", flex:1 }}>
          <span style={{ color:GOLD, fontWeight:900, fontSize:14, letterSpacing:3, textTransform:"uppercase" }}>MandaStrong</span>
