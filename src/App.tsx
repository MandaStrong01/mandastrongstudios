...
import Btn from "./Btn";
import { S, BG, BG2, BG3, GOLD, GOLD2, GOLDDIM, BORDER, TEXT, TEXT2, TEXT3 } from "./styles";
...
export default function App() {
  ...
  const TOTAL = 23; // Total pages
  const NAV = [
    "HOME", "INTRO", "UPLOADS", "LOGIN", "WORKSTATION", "VOICE", "IMAGES", "VIDEO", "MOTION", "ENHANCE",
    "TOOLS", "COMMUNITY", "TIMELINE", "AI EDIT", "STRIPE", "RENDER", "EXPORT", "SETTINGS", "HELP",
    "GROK", "COMMUNITY HUB", "THANK YOU", "END"
  ];

  const goTo = (n: number) => setPage(Math.max(0, Math.min(n, TOTAL - 1)));
  ...
  const handleFileUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setMediaLibrary(prev => [...prev, ...newAssets]);
  };

  const addAssetToLibrary = (asset: any) => setMediaLibrary(prev => [...prev, asset]);
                ...
                <div key={post.id} style={{ background: BG3, border: `1px solid ${BORDER}`, padding: 20 }}>
                  ...
                  <div style={{ borderTop: `1px solid ${BORDER}`, paddingTop: 10, marginTop: 4 }}>
                    ...
                    <div style={{ display: "flex", gap: 8, marginTop: 8 }}>
                      ...
                      }} style={{ ...S.btn("out"), fontSize: 9, padding: "6px 14px" }}>POST</button>
                    </div>
                  </div>
                </div>
              ))}
            ...
            <div style={{ padding: "40px 24px" }}>
              ...
              <div style={{ background: BG3, border: `1px solid ${GOLDDIM}`, padding: 36, maxWidth: 720, margin: "0 auto 28px", position: "relative" }}>
                ...
                <div style={{ fontSize: 13, lineHeight: 2.1, color: TEXT2 }}>
                  ...
                  <div style={{ marginTop: 20, fontFamily: "'Cinzel',serif", fontSize: 12, color: GOLD, lineHeight: 1.9 }}>
                    — Amanda Strong<br />Founder, MandaStrong Studio<br />Author, <em>Doxy the School Bully</em><br />
                    <a href="https://MandaStrong1.Etsy.com" target="_blank" rel="noopener noreferrer" style={{ color: GOLDDIM, textDecoration: "none" }}>MandaStrong1.Etsy.com</a>
                  </div>
                </div>
              </div>
            </div>
    ...
    <div style={{ display: "flex", flexDirection: "column", height: "100vh", background: BG, color: TEXT, fontFamily: "'Rajdhani',sans-serif", position: "relative" }}>
      ...
      <footer style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "7px 16px", background: BG2, borderTop: `1px solid ${GOLDDIM}`, flexShrink: 0, fontSize: 9, letterSpacing: 2, color: TEXT3, position: "relative", zIndex: 10, flexWrap: "wrap", gap: 6 }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
          <button onClick={() => goTo(page - 1)} disabled={page === 0} style={{ ...S.btn("out"), fontSize: 11, padding: "5px 14px", opacity: page === 0 ? 0.3 : 1 }}>◀ BACK</button>
          <span style={{ fontSize: 10, letterSpacing: 2, color: TEXT3, minWidth: 90, textAlign: "center" }}>PAGE {page + 1} / {TOTAL} — {NAV[page]}</span>
          <button onClick={() => goTo(page + 1)} disabled={page === TOTAL - 1} style={{ ...S.btn("out"), fontSize: 11, padding: "5px 14px", opacity: page === TOTAL - 1 ? 0.3 : 1 }}>NEXT ▶</button>
        </div>
        <div style={{ color: "#2ecc71" }}>⬤ AUTOSAVE ON &nbsp;•&nbsp; {mediaLibrary.length} ASSETS</div>
      </footer>

      {aiTool && <AIPanel tool={aiTool} onClose={() => setAiTool("")} onAssetCreated={addAssetToLibrary} />}
    </div>
  );
}