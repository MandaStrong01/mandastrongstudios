...
const autoFreeStorage=async()=>{
  try{
    ...
    // FIX: this used to delete the oldest CLIPS whenever storage passed 75%.
    // On iPad/Safari the quota is small, so a few 60s scenes tripped it and
    // Amanda's saved POC clips were silently destroyed on the next page load.
    // Source clips are now NEVER auto-deleted. Only superseded render files go.
    if(pct<0.90)return {freed:0,pct};
    const allClips=await getAllClipsFromDB();
    const clips=allClips.filter(c=>{
      const n=String(c.id)+" "+String(c.name||"");
      return /render_final_old|_film_|mandastrong_film/i.test(n);
    });
    if(clips.length===0)return {freed:0,pct,blocked:true};
