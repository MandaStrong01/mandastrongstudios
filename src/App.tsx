...
function speakText(voiceId, txt, onStart, onEnd) {
  ...
  const doSpeak = () => {
    ...
    if (voiceId==="james"||voiceId==="marcus"||voiceId==="river") {
      picked = allVoices.find(x=>/david|daniel|oliver|arthur|fred|tom|bruce|lee|ryan|eric/i.test(x.name))
    ...
    } else if (voiceId==="aurora") {
      picked = allVoices.find(x=>/kate|serena|emily/i.test(x.name))||allVoices.find(x=>x.lang==="en-GB");
