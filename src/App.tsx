...
const TEXT3  = "#555";
const BORDER = "#2a2a2a";
const GP: React.CSSProperties = { color: GOLD, fontWeight: 700 };
...
function Sparkles() {
  ...
  return (
    <div style={{ position: "absolute", inset: 0, overflow: "hidden", pointerEvents: "none" }}>
      {stars.map(s => (
        <div key={s.id} style={{ position: "absolute", left: s.x + "%", top: s.y + "%", width: s.size, height: s.size, borderRadius: "50%", background: GOLD2, opacity: 0, animation: `twinkle ${s.dur}s ${s.delay}s infinite`, boxShadow: `0 0 ${s.size * 2}px ${GOLD}` }} />
      ))}
      <style>{`
        @keyframes twinkle { 0%,100% { opacity:0; transform:scale(0.5); } 50% { opacity:1; transform:scale(1); } }
        @keyframes glowPulse { 0%,100% { text-shadow: 0 0 20px #d4a847, 0 0 40px #8a6d22; } 50% { text-shadow: 0 0 40px #f0c870, 0 0 80px #d4a847, 0 0 120px #8a6d22; } }
...
function GoldBtn({ onClick, children, style }: { onClick?: () => void; children: React.ReactNode; style?: React.CSSProperties }) {
  return (
    <button onClick={onClick} style={{ background: "linear-gradient(135deg, #8a6d22, #d4a847, #f0c870, #d4a847, #8a6d22)", backgroundSize: "200% auto", animation: "shimmer 3s linear infinite", color: "#000", fontWeight: 900, borderRadius: 50, padding: "14px 32px", border: "1px solid " + GOLD2, cursor: "pointer", fontSize: 15, letterSpacing: 2, textTransform: "uppercase" as const, boxShadow: "0 0 20px #d4a84766, 0 4px 15px rgba(0,0,0,0.5)", width: "100%", ...style }}>
      {children}
    </button>
  );
}
...
function PurBtn({ onClick, children, style }: { onClick?: () => void; children: React.ReactNode; style?: React.CSSProperties }) {
  return (
    <button onClick={onClick} style={{ background: "transparent", color: GOLD, fontWeight: 700, borderRadius: 50, padding: "14px 32px", border: "1.5px solid " + GOLD, cursor: "pointer", fontSize: 15, letterSpacing: 1, width: "100%", ...style }}>
      {children}
    </button>
  );
}

function Page({ children, sparkle }: { children: React.ReactNode; sparkle?: boolean }) {
  return (
    <div style={{ background: BG, color: TEXT, minHeight: "100vh", paddingTop: 52, position: "relative" }}>
      {sparkle && <Sparkles />}
...
function SH({ children }: { children: React.ReactNode }) {
  return <h2 style={{ color: GOLD, borderBottom: "1px solid " + GOLDDIM, fontSize: 20, fontWeight: 700, paddingBottom: 8, marginBottom: 16, letterSpacing: 1 }}>{children}</h2>;
}
...
function P1({ go }: { go: (n: number) => void }) {
  return (
    <div style={{ background: "radial-gradient(ellipse at center, #1a1200 0%, #0a0800 40%, #000 100%)", minHeight: "100vh", paddingTop: 52, position: "relative", overflow: "hidden" }}>
      <Sparkles />
      <div style={{ position: "absolute", top: "20%", left: "50%", transform: "translateX(-50%)", width: 300, height: 300, borderRadius: "50%", background: "radial-gradient(circle, #d4a84722 0%, transparent 70%)", pointerEvents: "none" }} />
      <div style={{ position: "relative", zIndex: 1, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", minHeight: "calc(100vh - 52px)", padding: "0 24px", textAlign: "center" }}>
        <div style={{ fontSize: 48, marginBottom: 8, filter: "drop-shadow(0 0 12px #d4a847)" }}>✦</div>
        <div style={{ color: TEXT3, fontSize: 11, letterSpacing: 4, textTransform: "uppercase", marginBottom: 12 }}>Cinema Intelligence Platform</div>
