...
const STOCK_VOICES = [
  { id:"aurora", name:"Aurora", desc:"Warm British Female", style:"Documentary · Narrator", accent:"British RP", pitch:1.0, rate:0.9, moods:["Warm","Authoritative","Measured","Cinematic","Serious"] },
  { id:"marcus", name:"Marcus", desc:"Deep American Male", style:"Cinematic · Authoritative", accent:"American", pitch:0.85, rate:0.95, moods:["Powerful","Dramatic","Commanding","Epic","Intense"] },
  { id:"sophia", name:"Sophia", desc:"Bright Australian Female", style:"Upbeat · Engaging", accent:"Australian", pitch:1.1, rate:1.05, moods:["Upbeat","Friendly","Energetic","Playful","Cheerful"] },
  { id:"james", name:"James", desc:"Dry British Male", style:"Sarcastic · Witty", accent:"British", pitch:0.92, rate:0.88, moods:["Sarcastic","Witty","Dry","Deadpan","Sardonic"] },
  { id:"nova", name:"Nova", desc:"Neutral AI Female", style:"Clean · Professional", accent:"Neutral", pitch:1.05, rate:1.0, moods:["Professional","Neutral","Clear","Precise","Calm"] },
  { id:"river", name:"River", desc:"Warm American Male", style:"Friendly · Intimate", accent:"American South", pitch:0.95, rate:0.92, moods:["Warm","Intimate","Conversational","Storytelling","Soulful"] },
];
...
function MusicVideoStudio({ onClose, onSave }) {
  ...
  const [config, setConfig] = useState({title:"",artist:"",genre:"",mood:"",tempo:"",structure:"",vocals:"",instruments:[],vocalStyle:"",lyrics:"",lyricsMode:"write",videoStyle:"",colorGrade:"",effects:[],cuts:"",aspectRatio:"16:9",duration:"",extras:[]});
  const set = (k,v) => setConfig(p=>({...p,[k]:v}));
  const toggle = (k,v) => setConfig(p=>({...p,[k]:p[k].includes(v)?p[k].filter(x=>x!==v):[...p[k],v]}));
  const GENRES=["Pop","Rock","Hip Hop","R&B / Soul","Electronic / EDM","Country","Jazz","Classical","Metal","Reggae","Folk / Acoustic","Latin","K-Pop","Drill","Trap","Afrobeats","Gospel","Blues","Cinematic / Score"];
  ...
  const INSTRUMENTS=["Electric Guitar","Acoustic Guitar","Bass Guitar","Piano / Keys","Synthesizer","Drums / Percussion","Violin / Strings","Trumpet / Brass","Saxophone","Flute","808 Bass","TR-808 Drums","Choir Pad","Orchestra","Banjo","Ukulele","Harp"];
  ...
  const COLOR_GRADES=["Natural / Clean","Golden Hour Warm","Cool Blue / Moody","High Contrast B&W","Neon / Vivid","Pastel / Soft","Cinematic Teal & Orange","Vintage Film Grain","Dark & Desaturated","Hyper Colour Pop"];
  const EFFECTS=["Slow Motion","Speed Ramps","Glitch Effects","Light Leaks","Lens Flares","Rain / Water","Fire / Smoke","Bokeh / Blur","Double Exposure","Mirror / Kaleidoscope","Grain / Noise","Vignette","Chromatic Aberration","Particle Effects"];
...
${config.lyrics}`:"LYRICS: Generate original lyrics matching genre and mood"}

Provide:
1. Song concept & artist vision
2. Complete original lyrics
3. Music production notes
4. Video treatment scene by scene
5. Shot list (10+ shots)
6. Post production & colour notes
    ...
    <div style={{position:"fixed",inset:0,zIndex:1100,background:"rgba(0,0,0,0.97)",display:"flex",alignItems:"center",justifyContent:"center"}}>
      <div style={{width:"min(820px,98vw)",background:"#050505",border:`2px solid ${GOLD}`,maxHeight:"95vh",overflowY:"auto",display:"flex",flexDirection:"column"}}>
        ...
        <div style={{padding:"20px 24px",flex:1}}>
          ...
          {step===4&&result&&(<div><div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:14}}><div style={{fontFamily:"'Cinzel',serif",color:GOLD,fontSize:16,fontWeight:900,letterSpacing:3}}>✦ YOUR MUSIC VIDEO PRODUCTION PACKAGE</div><div style={{display:"flex",gap:8}}><button onClick={()=>{if(onSave)onSave({id:Date.now()+Math.random(),name:`Music Video — ${config.title||"Untitled"}`,type:"text/plain",url:"",content:result});alert("Saved to Media Library!");}} style={{...G("gold",true)}}>SAVE TO LIBRARY</button><button onClick={()=>{setResult(null);setStep(1);}} style={{...G("out",true)}}>NEW PROJECT</button></div></div><textarea value={result} readOnly style={{...inp,height:420,resize:"none",lineHeight:1.8,fontSize:13}}/></div>)}
        </div>
...
function P6Voice({ onSave }) {
  ...
  useEffect(()=>{
    const load=()=>{ const v=window.speechSynthesis.getVoices(); if(v.length>0)setAllDeviceVoices(v); };
    load(); window.speechSynthesis.onvoiceschanged=load; setTimeout(load,800);
  },[]);

  const assignVoice=(charId,voiceName)=>{
    const updated={...voiceAssign,[charId]:voiceName};
    setVoiceAssign(updated); VOICE_ASSIGNMENTS=updated;
  ...
  const speak=(voiceId,txt)=>{
    ...
    window.speechSynthesis.cancel();
    const sv=STOCK_VOICES.find(x=>x.id===voiceId);
    const allVoices=window.speechSynthesis.getVoices();
    const utt=new SpeechSynthesisUtterance(txt.replace(/\[pause\]/g,". ").replace(/[*\/]/g," ").slice(0,5000));
    utt.pitch=sv?sv.pitch:1.0; utt.rate=sv?sv.rate:0.9;
    ...
    if(!picked){
      ...
      if(isMale){
        picked=allVoices.find(x=>malePat.test(x.name))||allVoices.find(x=>x.lang==="en-GB"&&!femalePat.test(x.name))||allVoices.find(x=>x.lang.startsWith("en")&&!femalePat.test(x.name))||allVoices[0];
      }else{
  ...
  const generateNarration=async()=>{
    ...
    const sv=STOCK_VOICES.find(x=>x.id===selVoice);
    const moodNote=selMood?` Deliver in a ${selMood} mood.`:"";
    try{
      const res=await fetch("https://api.anthropic.com/v1/messages",{method:"POST",headers:{"Content-Type":"application/json","anthropic-dangerous-direct-browser-access":"true"},body:JSON.stringify({model:"claude-sonnet-4-20250514",max_tokens:1000,messages:[{role:"user",content:`Professional narrator, voice style: ${sv?.style} — ${sv?.desc}.${moodNote} Format this as cinematic narration with [pause] for pauses:
...
${text}`}]})});
  ...
  const saveToLibrary=()=>{
    const content=result||text; if(!content.trim())return;
    const v=STOCK_VOICES.find(x=>x.id===selVoice);
    if(onSave)onSave({id:Date.now()+Math.random(),name:`Narration — ${v?.name}${selMood?" ("+selMood+")":""}`,type:"audio/narration",url:"",content});
    setSaved(true);
  };
    ...
    <div style={{...Sp}}>
      ...
      <div style={{padding:"16px 18px"}}>
        ...
        <div style={{...Card(),marginBottom:20,border:`1px solid ${GOLD}`,background:"#050505"}}>
          ...
          <div style={{color:WHITE,fontSize:12,marginBottom:14}}>Test any device voice below, then assign it to a character. Your assignments are saved automatically.</div>
          <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(220px,1fr))",gap:6,marginBottom:14,maxHeight:260,overflowY:"auto"}}>
            {allDeviceVoices.map(v=>(<div key={v.name} style={{background:"#000",border:`1px solid ${GOLDDIM}`,padding:"8px 10px"}}>
              ...
              <div style={{display:"flex",gap:4,flexWrap:"wrap"}}>
                ...
                {STOCK_VOICES.map(sv=>(<button key={sv.id} onClick={()=>assignVoice(sv.id,v.name)} style={{fontSize:9,padding:"2px 8px",background:voiceAssign[sv.id]===v.name?GOLD:"#111",border:`1px solid ${voiceAssign[sv.id]===v.name?"#000":GOLDDIM}`,color:voiceAssign[sv.id]===v.name?"#000":WHITE,cursor:"pointer",fontWeight:900,letterSpacing:1}}>{sv.name}</button>))}
              </div>
            </div>))}
          </div>
          <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:8}}>
            {STOCK_VOICES.map(sv=>(<div key={sv.id} style={{background:"#0a0a0a",border:`1px solid ${voiceAssign[sv.id]?GOLD:GOLDDIM}`,padding:"8px 10px"}}>
              <div style={{color:GOLD,fontSize:11,fontWeight:900,letterSpacing:2,marginBottom:3}}>{sv.name}</div>
              <div style={{color:voiceAssign[sv.id]?WHITE:DIM,fontSize:10,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{voiceAssign[sv.id]||"auto-matched"}</div>
              {voiceAssign[sv.id]&&<button onClick={()=>assignVoice(sv.id,"")} style={{background:"none",border:"none",color:"#ef4444",cursor:"pointer",fontSize:9,padding:0,marginTop:2}}>✕ clear</button>}
            </div>))}
          </div>
        </div>

        {/* SELECT VOICE */}
        <div style={{color:GOLD,fontSize:12,letterSpacing:3,fontWeight:900,marginBottom:12}}>SELECT VOICE & MOOD</div>
        <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:8,marginBottom:16}}>
          {STOCK_VOICES.map(v=>(<div key={v.id} onClick={()=>{setSelVoice(v.id);setSelMood(null);}}
            ...
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:6}}>
              ...
              <button onClick={e=>{e.stopPropagation();speak(v.id,`Hi, I am ${v.name}. ${v.desc}. Ready to narrate.`);}} style={{background:"none",border:`1px solid ${GOLDDIM}`,color:GOLD,padding:"3px 10px",cursor:"pointer",fontSize:11,fontWeight:900}}>{playing===v.id?"⏹ STOP":"▶ PLAY"}</button>
            </div>
            <div style={{color:GOLD,fontSize:11,letterSpacing:1,marginBottom:3}}>{v.desc}</div>
            <div style={{color:WHITE,fontSize:10,marginBottom:6}}>{v.style} · {v.accent}</div>
            {voiceAssign[v.id]&&<div style={{color:GOLDDIM,fontSize:9,marginBottom:6,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>🎙 {voiceAssign[v.id]}</div>}
            {selVoice===v.id&&(
              <div style={{marginTop:6,paddingTop:6,borderTop:`1px solid ${GOLDDIM}`}}>
                <div style={{color:GOLD,fontSize:9,letterSpacing:2,marginBottom:5,fontWeight:900}}>MOOD / DELIVERY</div>
                <div style={{display:"flex",flexWrap:"wrap",gap:4}}>
                  {v.moods.map(m=>(<button key={m} onClick={e=>{e.stopPropagation();setSelMood(selMood===m?null:m);}} style={{fontSize:9,padding:"2px 7px",background:selMood===m?GOLD:"#111",border:`1px solid ${selMood===m?"#000":GOLDDIM}`,color:selMood===m?"#000":WHITE,cursor:"pointer",fontWeight:900,letterSpacing:1}}>{m}</button>))}
                </div>
              </div>
            )}
          </div>))}
        ...
        {selMood&&<div style={{background:"#0a0500",border:`1px solid ${GOLD}`,padding:"8px 14px",marginBottom:16,color:GOLD,fontSize:12,fontWeight:900,letterSpacing:2}}>✦ MOOD: {selMood.toUpperCase()} · Voice: {STOCK_VOICES.find(x=>x.id===selVoice)?.name}</div>}

        {/* TEXT TO NARRATION */}
        <div style={{...Card(),marginBottom:16}}>
          <div style={{color:GOLD,fontSize:12,letterSpacing:3,fontWeight:900,marginBottom:10}}>TEXT TO NARRATION / VOICEOVER</div>
          <div style={{color:WHITE,fontSize:13,marginBottom:12}}>Voice: <strong style={{color:GOLD}}>{STOCK_VOICES.find(x=>x.id===selVoice)?.name}</strong> — {STOCK_VOICES.find(x=>x.id===selVoice)?.desc}{selMood&&<span style={{color:GOLDDIM}}> · Mood: {selMood}</span>}</div>
          ...
          <div style={{display:"flex",gap:10}}>
            <button onClick={generateNarration} disabled={loading||!text.trim()} style={{...G("gold",false),flex:1,padding:"12px",opacity:loading||!text.trim()?0.5:1}}>{loading?"⟳ GENERATING...":"▶ GENERATE & SPEAK"}</button>
            <button onClick={()=>speak(selVoice,text)} disabled={!text.trim()} style={{...G("out",false),padding:"12px 20px",opacity:!text.trim()?0.5:1}}>▶ SPEAK NOW</button>
          </div>
        </div>

        {result&&(<div style={{...Card(),marginBottom:16}}>
          <div style={{color:GOLD,fontSize:12,letterSpacing:3,fontWeight:900,marginBottom:8}}>NARRATION OUTPUT</div>
          <textarea value={result} onChange={e=>setResult(e.target.value)} style={{...inp,height:120,resize:"none",lineHeight:1.7,marginBottom:10}}/>
          <div style={{display:"flex",gap:10}}>
